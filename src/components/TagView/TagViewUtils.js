...
export function addTagView (to) {
  ...
  if (t.title !== null && t.title !== undefined && t.fullPath !== '/' && t.fullPath.indexOf('#') === -1) {
    store().commit('skin/ADD_TAG_VIEW', t)
  }
}

export function setTagView (tagView) {
  store().commit('skin/SET_TAG_VIEW', tagView)
}

// 只移除一个 tagView
export function removeATagView (state, payload) {
  let _this = Vue.prototype.$app
  // 记录被移除的路由
  // console.log('-------------_this.$route.path------------------',_this,_this.router.history.current.fullPath)
  ...
  if (state.tagView.length === 0) {
    ...
    if (payload === state.tagView.length && _this.router.history.current.fullPath === removedTagView) {
      router.push(state.tagView[payload - 1].fullPath)
      return
    }
    // 如果移除的是第一个 tagView 则路由跳转移除后的第一个 tagView
    if (payload === 0 && _this.router.history.current.fullPath === removedTagView) {
      router.push(state.tagView[0].fullPath)
      return
    }
    if (_this.router.history.current.fullPath === removedTagView) {
      router.push(state.tagView[payload - 1].fullPath)
    }
  }
}
