  ...
  <div>
    ...
    <template v-for="(item,index) in myRouter">
      <template v-if="item.hidden !== true">
        <!-- <q-item-label v-if="item.meta && item.meta.title"
           header
        ...
        >
          <q-item-section avatar>
            <q-icon :name="(item.meta && item.meta.hasOwnProperty('icon')) ? item.meta.icon : ''" />
          </q-item-section>
          <q-item-section>
            {{(item.meta && item.meta.hasOwnProperty('title')) ? item.meta.title : ''}}
          </q-item-section>
        </q-item>
        <!-- 一个孩子 -->
        <q-item v-else-if="hasOneShowingChild(item.children,item)"
           clickable
           v-ripple
           :key="index"
           :exact="item.children[0].path === '/'"
           :class="baseItemClass"
           :inset-level="initLevel"
           :style="isWeChart?' line-height: normal':''"
           active-class="baseItemActive"
           :to="handleLink(basePath, item.children[0].path)"
           @click="externalLink(basePath, item.children[0].path)"
        >
          <q-item-section avatar>
            <q-icon :name="(item.children[0].meta && item.children[0].meta.hasOwnProperty('icon')) ? item.children[0].meta.icon : ''" />
          </q-item-section>
          <q-item-section>
            {{(item.children[0].meta && item.children[0].meta.hasOwnProperty('title')) ? item.children[0].meta.title : ''}}
          </q-item-section>
        </q-item>
        ...
        <q-expansion-item v-else
           ...
           :default-opened="(item.meta && item.meta.hasOwnProperty('isOpen')) ? item.meta.isOpen : false "
           :header-inset-level="initLevel"
           :key="index"
           :icon="(item.meta && item.meta.hasOwnProperty('icon')) ? item.meta.icon : '' "
           :label="(item.meta && item.meta.hasOwnProperty('title')) ? item.meta.title : ''"
           :style="isWeChart?' line-height: normal':''"
        >
