...
import { removeATagView, removeOneSide } from '@/components/TagView/TagViewUtils'
    ...
    // 设置用户类型，并根据权限获取授权路由
  export function  SET_ROLES_AND_ROUTES(state, payload){
      state.role = payload
      // 深拷贝
  ...
  export function LOGOUT(state, payload){
    ...
    // 新增 tagView
  export function  ADD_TAG_VIEW(state, payload){
  ...
  export function  SET_TAG_VIEW(state, payload){
     ...
     * @constructor
     */
  export function  REMOVE_TAG_VIEW(state, payload){
  ...
  export function  SET_BREADCRUMBS(state, payload){
     ...
     */
  export function  SET_KEEPALIVE_LIST(state, payload) {
      state.keepAliveList = []
